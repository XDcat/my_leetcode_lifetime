...
class Node {
    ...
    public static int dfs(Node root){
        ...
        if (left != null){
            root.left = null;
            n1 = findBlock(root, !root.isRed);
            n2 =  findBlock(left, !left.isRed) ;
            root.left = left;
            res += Math.abs(n1 - n2);
        }
        ...
        if (right != null){
            root.right = null;
            n1 = findBlock(root, !root.isRed);
            n2 = findBlock(right, !right.isRed);
            root.right = right;
            res += Math.abs(n1 - n2);
        }

        int nl = dfs(root.left);
        int nr = dfs(root.right);
        return res + nl + nr;
    }

    public static int findBlock(Node root, boolean preIsRed) {
        if (root == null) {
            return 0;
        }

        int cur = (preIsRed != root.isRed) ? 1 : 0;
        return cur + findBlock(root.left, root.isRed) + findBlock(root.right, root.isRed);
    }
